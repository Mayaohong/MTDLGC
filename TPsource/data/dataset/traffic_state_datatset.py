...
from TPsource.data.dataset import AbstractDataset
from TPsource.data.utils import generate_dataloader, cal_corr
from TPsource.utils import StandardScaler, NormalScaler, NoneScaler, \
...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.cache_file_folder = './TPsource/cache/dataset_cache/'
        ...
        self.smi_weight_adj = self.config.get('smi_weight_adj', False)
        ...
        if os.path.exists(self.data_path + self.dyna_file + '.dyna'):
            self._load_dyna_data()
        else:
            raise ValueError('Not found .dyna file!')
        if os.path.exists(self.data_path + self.rel_file + '.rel'):  # .rel file is not necessary
            self._load_rel()
        else:
            self.adj_mx = np.zeros((len(self.geo_ids), len(self.geo_ids)), dtype=np.float32)  # initialize the adj_mx
        
        #已经得到adj_mx
        if self.model_name =="MTSLGC" or self.model_name == "MTDLGC":
            # print("Computing a multi-hop adj matrix:>>>")
            # self.K = config.get('K', 3)
            # # get G from adj
            # G = nx.from_numpy_array(self.adj_mx)
            # self.supports = [self.adj_mx]
            # for k in range(2,self.K+1):#k = 2,3,4,5
            #     temp_adj = np.zeros([self.num_nodes, self.num_nodes])
            #     for i in range(self.num_nodes):
            #         for j in range(self.num_nodes):
            #             if i != j: 
            #                 try:
            #                     path = nx.shortest_path(G, source=i, target=j)
            #                     if len(path) == k + 1:#存在k邻居, k>=2
            #                         temp_adj[i, j] = 1.0
            #                 except nx.NetworkXNoPath:
            #                     continue
            #     self.supports.append(temp_adj)
            # self.adj_mx = self.supports
            print("loading a multi-hop adj matrix:>>>")
            if self.distance_inverse and self.set_weight_link_or_dist.lower() != 'link':
                self.weighting_type = 'time'
            elif self.calculate_weight_adj and self.set_weight_link_or_dist.lower() != 'link':
                self.weighting_type = 'gauss'
            elif self.smi_weight_adj and self.set_weight_link_or_dist.lower() != 'link':
                self.weighting_type = 'smi'
            else:# self.set_weight_link_or_dist.lower() == 'link'
                self.weighting_type = '0-1'
            self.supports = []
            for k in range(1,self.K+1):#k = 2,3,4,5
                self.adj_path = './raw_data/'+self.dataset+'/adj_'+self.dataset.lower()+'_'+str(k)+'_'+ self.weighting_type+'.csv'
                tem_adj = pd.read_csv(self.adj_path, header=None)
                self.supports.append(np.array(tem_adj))
            self.adj_mx = self.supports
    ...
    def _load_dyna_data(self):
        """
        加载.dyna文件，格式[dyna_id, type, time, entity_id, properties(若干列)],
        .geo文件中的id顺序应该跟.dyna中一致,
        其中全局参数`data_col`用于指定需要加载的数据的列，不设置则默认全部加载
        Returns:
            np.ndarray: 数据数组, 2d-array: (num_nodes,len_time)
        """
        # 加载数据集
        self._logger.info("Loading file " + self.dyna_file + '.dyna')
        ...
        if self.data_col != '':  # 根据指定的列加载数据集
            if isinstance(self.data_col, list):
                ...
                data_col = [self.data_col].copy()
            data_col.insert(0, 'entity_id')
            dynafile = dynafile[data_col]
        else:  # 不指定则加载所有列
            dynafile = dynafile[dynafile.columns[3:]]  # 从time列开始所有列
        #只取3天的数据
        # 按照entity_id对traffic_flow进行分组，并将每组中的traffic_flow列合并为一个list
        if 'traffic_flow' in dynafile.columns:
            grouped_dynafile = dynafile.groupby(['entity_id'])['traffic_flow'].apply(list)
        elif 'traffic_speed' in dynafile.columns:
            grouped_dynafile = dynafile.groupby(['entity_id'])['traffic_speed'].apply(list)
        else:
            print('Attribute fields is not found')

        # 构造len(self.geo_ids)行的时间序列矩阵，每一行代表一个entity_id的traffic_flow时间序列
        #只取1天的数据
        self.time_series_matrix = np.zeros((len(self.geo_ids), 288))  # [num_nodes, time_length]
        for geo_ids in self.geo_ids:
            self.time_series_matrix[self.geo_to_ind[geo_ids], :] = grouped_dynafile.loc[geo_ids][-288:]
    ...
    def _load_rel(self):
        ...
        if self.weight_col != '':  # 根据weight_col确认权重列
            ...
            else:  # len(relfile.columns) >= 5, properties只有一列或多列，，那就默认最后一列是权重列
        ...
        for i in range(self.adj_mx.shape[0]):
            for j in range(self.adj_mx.shape[1]):
                if self.adj_mx[i, j] !=0:
                    gauss_value= np.exp(-np.square(self.adj_mx[i, j]*self.adj_mx[j, i]/ std))
                    self.adj_mx[i, j] = gauss_value
    ...
    def _distance_inverse(self):
        ...
        self.adj_mx[np.isinf(self.adj_mx)] = 0

    def _smi_adjacency_matrix(self):
        self._logger.info("Start Calculate the weight by similarity!")
        # 1st adjacent matrix
        for i in range(len(self.geo_ids)):
            for j in range(len(self.geo_ids)):
                if self.adj_mx[i, j] != 0:
                    self.adj_mx[i, j] = cal_corr(self.time_series_matrix[i], self.time_series_matrix[j], 288)
    ...
    def _load_dyna_3d(self, filename):
        ...
        # 求时间序列
        self.timesolts = list(dynafile['time'][:int(dynafile.shape[0] / len(self.geo_ids))])  # len(timeslots)=多少天*288
        ...
        if not dynafile['time'].isna().any():  # 时间没有空值
            ...
            print("len of timesolts:", len(self.timesolts))
            for idx, _ts in enumerate(self.timesolts):
                self.idx_of_timesolts[_ts] = idx
        # 转3-d数组
        feature_dim = len(dynafile.columns) - 2  # not consider dyna_id,type
        df = dynafile[dynafile.columns[-feature_dim:]]
        len_time = len(self.timesolts)  # each node concluding the length of time series
        data = []
    ...
    def _generate_input_data(self, df):
        ...
        for t in range(min_t, max_t):
            # x: (num_samples, input_length, ..., input_dim)    [num_samples, input_window, num_nodes, input_dim]
    ...
    def _generate_data(self):
        ...
        for filename in data_files:
            df = self._load_dyna(filename)  # (len_time, num_nodes, feature_dim)
            if self.load_external:
                df = self._add_external_information(df, ext_data)
            x, y = self._generate_input_data(df)  # 优先使用子类中的同名函数
            # x: (num_samples, input_length, ..., input_dim) ; (num_samples, input_length(例如36), num_nodes, input_dim)
            # y: (num_samples, output_length(例如12), ..., output_dim);
            x_list.append(x)  # 为了 np.concatenate
            y_list.append(y)
    ...
    def _split_train_val_test(self, x, y):
        ...
        """
        # if (self.dataset == "PEMSD8" and self.dataset == "PEMSD4") and (self.model_name == "MTDLGC" or self.model_name == "MTLGC"):
    ...
    def get_data(self):
        ...
        self.ext_dim = self.feature_dim - self.output_dim  # 如何理解
