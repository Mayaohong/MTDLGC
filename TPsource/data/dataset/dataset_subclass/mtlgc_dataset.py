...
MTLGCDataset既可以继承TrafficStatePointDataset，也可以继承TrafficStateGridDataset以处理网格数据
...
class MTLGCDataset(TrafficStatePointDataset):
    def __init__(self, config):
        super().__init__(config)
        self.points_per_hour = 3600 // self.time_intervals  # self.time_intervals来自于继承，每小时的时间片数
        self.len_closeness = self.config.get('len_closeness', 1)
        self.len_period = self.config.get('len_period', 1)
        self.len_trend = self.config.get('len_trend', 1)
        ...
        self.dynafile = pd.read_csv(self.data_path + self.dyna_file + '.dyna')
        self.parameters_str = \
    ...
    def _search_data(self, sequence_length, label_start_idx, output_window, num_of_depend, units):
        """
        ...
        if label_start_idx + output_window > sequence_length:
            return None
        x_idx = []
        for i in range(1, num_of_depend + 1):
            # 从label_start_idx向左偏移，i是区间数，units*points_per_hour是区间长度(时间片为单位)； self.points_per_hour 每小时多少个区间（12）
            start_idx = label_start_idx - self.points_per_hour * units * i  # 12*[7*24 or 1]*[1,2,3]
            end_idx = start_idx + output_window
            if start_idx >= 0:
                x_idx.append((start_idx, end_idx))  # 每一段的长度是output_window
            else:
                return None
    ...
    def _search_data_daily(self, sequence_length, label_start_idx, output_window, num_of_depend, units):
        """
        根据全局参数len_closeness/len_period/len_trend找到数据索引的位置
        ...
        for i in range(1, num_of_depend + 1):
            # 从label_start_idx向左偏移，i是区间数，units*points_per_hour是区间长度(时间片为单位)； self.points_per_hour 每小时多少个区间（12）
            start_idx = label_start_idx - self.points_per_hour * units * i  # 288*[1,2,3]
            if datetime.strptime(self.dynafile.iloc[label_start_idx, 2], '%Y-%m-%dT%H:%M:%SZ').weekday() == 0:
                start_idx = start_idx - 288 * 3
            elif datetime.strptime(self.dynafile.iloc[label_start_idx, 2], '%Y-%m-%dT%H:%M:%SZ').weekday() == 5:
                start_idx = start_idx - 288 * 6
            else:
                start_idx = start_idx
            end_idx = start_idx + output_window
            if start_idx >= 0:
    ...
    def _get_sample_indices(self, data_sequence, label_start_idx):
        ...
        if self.len_period > 0:
            period_indices = self._search_data_daily(data_sequence.shape[0], label_start_idx, self.output_window,
