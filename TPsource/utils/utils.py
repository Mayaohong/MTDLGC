...
def get_executor(config, model, data_feature):
    ...
    """
    try:
        # importlib在程序运行时动态地导入模块，而不需要提前知道模块名
        # getattr用于获取对象的公共属性和方法，无法获取私有属性和方法
        return getattr(importlib.import_module('TPsource.executor'),
                       config['executor'])(config, model, data_feature)  # 加载后初始化方法
...
def get_model(config, data_feature):
    ...
    if config['task'] == 'traj_loc_pred':
        try:
            return getattr(importlib.import_module('TPsource.model.trajectory_loc_prediction'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'traffic_state_pred':
        try:
            return getattr(importlib.import_module('TPsource.model.traffic_flow_prediction'),
                           config['model'])(config, data_feature)
        except AttributeError:
            try:
                return getattr(importlib.import_module('TPsource.model.traffic_speed_prediction'),
                               config['model'])(config, data_feature)
            except AttributeError:
                try:
                    return getattr(importlib.import_module('TPsource.model.traffic_demand_prediction'),
                                   config['model'])(config, data_feature)
                except AttributeError:
                    try:
                        return getattr(importlib.import_module('TPsource.model.traffic_od_prediction'),
                                       config['model'])(config, data_feature)
                    except AttributeError:
                        try:
                            return getattr(importlib.import_module('TPsource.model.traffic_accident_prediction'),
                                           config['model'])(config, data_feature)
                        except AttributeError:
                            raise AttributeError('model is not found')
    elif config['task'] == 'map_matching':
        try:
            return getattr(importlib.import_module('TPsource.model.map_matching'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'road_representation':
        try:
            return getattr(importlib.import_module('TPsource.model.road_representation'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'eta':
        try:
            return getattr(importlib.import_module('TPsource.model.eta'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    else:
...
def get_evaluator(config):
    ...
    """
    try:
        return getattr(importlib.import_module('TPsource.evaluator'),
...
def get_logger(config, name=None):
    ...
    # asctime 表示日志记录的时间；levelname 表示日志级别；message 表示日志消息内容。
...
def preprocess_data(data, config):
    ...
    """
    train_rate = config.get('train_rate', 0.6)
    eval_rate = config.get('eval_rate', 0.2)
...
def set_random_seed(seed):
    """
    用于设置随机数种子，以保证随机数在不同的运行环境中具有一致性
    torch.backends.cudnn.deterministic 设置为 True，以使得在使用 CUDA 的情况下，每次结果都是确定性的。
    Args:
        seed(int): 种子数
    """
    random.seed(seed)
    ...
    torch.backends.cudnn.deterministic = False  # 将PyTorch中的CUDA 随机数生成器设为确定性随机数生成器，从而使其生成的随机数序列也是相同的。
