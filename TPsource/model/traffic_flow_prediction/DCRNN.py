...
def calculate_normalized_laplacian(adj):
    ...
    normalized_laplacian = np.matmul(np.matmul(sinvd, adj), sinvd)  # n*n
...
def calculate_random_walk_matrix(adj_mx):
    ...
    random_walk_mx = np.matmul(d_mat_inv, adj_mx)
...
def calculate_scaled_laplacian(adj_mx, lambda_max=2, undirected=True):
    ...
    diag = np.diag(np.sum(lap, axis=1))
    lap = diag - lap
    lambda_max = np.linalg.eigvals(lap).max().real
    return (2 * lap) / lambda_max - np.identity(lap.shape[0])

def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)

class GCONV(nn.Module):
    def __init__(self, config, num_nodes, max_diffusion_step, supports, device, input_dim, hid_dim, output_dim, bias_start=0.0):
        super().__init__()
    ...
    def forward(self, inputs, state):
        ...
        inputs_and_state = torch.cat([inputs, state], dim=2) # (B, N, total_arg_size(input_dim+state_dim))
        ...
        if self._max_diffusion_step == 0:
            ...
            for support in self._supports:
                ...
                # T1=L x1=T1*x=L*x
                x1 = torch.matmul(laplacian, x0)  # supports: n*n; x0: n*(total_arg_size * batch_size)
...
class DCGRUCell(nn.Module):
    def __init__(self, config, input_dim, num_units, supports, max_diffusion_step, num_nodes, device, nonlinearity='tanh',
                use_gc_for_ru=True):
        """
        Args:
            input_dim:
        ...
        if self._use_gc_for_ru:
            self._fn = GCONV(config, self._num_nodes, self._max_diffusion_step, self._supports, self._device,
        ...
        self._gconv = GCONV(config, self._num_nodes, self._max_diffusion_step, self._supports, self._device,
...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, supports):
        nn.Module.__init__(self)
        Seq2SeqAttrs.__init__(self, config)
        self.dcgru_layers = nn.ModuleList()
        self.dcgru_layers.append(DCGRUCell(config, self.input_dim, self.rnn_units, supports, self.max_diffusion_step,
                                           self.num_nodes, self.device))
        for i in range(1, self.num_rnn_layers):
            self.dcgru_layers.append(DCGRUCell(config, self.rnn_units, self.rnn_units, supports, self.max_diffusion_step,
                                               self.num_nodes, self.device))
...
class DecoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, supports):
        ...
        self.dcgru_layers = nn.ModuleList()
        self.dcgru_layers.append(DCGRUCell(config, self.output_dim, self.rnn_units, supports, self.max_diffusion_step,
                                           self.num_nodes, self.device))
        for i in range(1, self.num_rnn_layers):
            self.dcgru_layers.append(DCGRUCell(config, self.rnn_units, self.rnn_units, supports, self.max_diffusion_step,
                                               self.num_nodes, self.device))
...
class DCRNN(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        ...
        if self.filter_type == "laplacian":
            supports.append(torch.tensor(calculate_scaled_laplacian(self.adj_mx, lambda_max=None)).type(torch.float32).to(self.device))
        elif self.filter_type == "random_walk":
            supports.append(torch.tensor(calculate_random_walk_matrix(self.adj_mx).T).type(torch.float32).to(self.device))
        elif self.filter_type == "dual_random_walk":
            supports.append(torch.tensor(calculate_random_walk_matrix(self.adj_mx).T).type(torch.float32).to(self.device))
            supports.append(torch.tensor(calculate_random_walk_matrix(self.adj_mx.T).T).type(torch.float32).to(self.device))
        else:
            supports.append(torch.tensor(calculate_scaled_laplacian(self.adj_mx, lambda_max=None)).type(torch.float32).to(self.device))
        for support in supports:
            self._supports.append(support)

        self.encoder_model = EncoderModel(config, self._supports)
        self.decoder_model = DecoderModel(config, self._supports)
    ...
    def forward(self, batch, batches_seen=None):
        ...
        outputs = outputs.view(self.output_window, batch_size, self.num_nodes, self.output_dim).permute(1, 0, 2, 3) #[B,O,N,out_dim]
        return outputs
