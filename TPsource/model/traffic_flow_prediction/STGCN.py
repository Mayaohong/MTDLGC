...
def calculate_cheb_poly(lap, ks):
    ...
    if ks == 1:
        return np.asarray(lap_list[0:1])  # 1*n*n 返回W对角线就是全1
    else:
...
class TemporalConvLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x_in = self.align(x)[:, :, self.kt - 1:, :]  # (batch_size, c_out, input_length-kt+1, num_nodes) 进行残差连接
        if self.act == "GLU":
            # x: (batch_size, c_in, input_length, num_nodes)
            x_conv = self.conv(x)
            # x_conv: (batch_size, c_out * 2, input_length-kt+1, num_nodes)  [P Q] 分两半
...
class SpatioConvLayer(nn.Module):
    def __init__(self, config, ks, c_in, c_out, lk, w_adj, device):
        ...
        self.theta = torch.nn.Parameter(torch.zeros((c_in, c_out, ks), device= device))
        self.theta2 = torch.nn.Parameter(torch.zeros((c_in, c_out), device= device))
        self.b = torch.nn.Parameter(torch.zeros((1, c_out, 1, 1), device= device))

        # self.theta = nn.Parameter(torch.FloatTensor(c_in, c_out, ks).to(device))  # kernel: C_in*C_out*ks
        # self.theta2 = nn.Parameter(torch.FloatTensor(c_in, c_out).to(device))  # kernel: C_in*C_out*ks
        # self.b = nn.Parameter(torch.FloatTensor(1, c_out, 1, 1).to(device))
        self.align = Align(c_in, c_out)
        self.reset_parameters()

    def reset_parameters(self):
        torch.nn.init.xavier_normal_(self.theta)
    ...
    def forward(self, x):
        ...
        laplacian = self.Lk * self.w_adj + self.Lk
        x_c = torch.einsum("knm,bitm->bitkn", laplacian, x)  # delete num_nodes(n) [B,C_in ,L, Ks, N]
...
class STConvBlock(nn.Module):
    def __init__(self, config, ks, kt, n, c, p, lk, w_adj, device):
        super(STConvBlock, self).__init__()
        self.tconv1 = TemporalConvLayer(kt, c[0], c[1], "GLU")
        self.sconv = SpatioConvLayer(config, ks, c[1], c[1], lk, w_adj, device)
    ...
    def forward(self, x):  # x: (batch_size, feature_dim/c[0], input_length, num_nodes)
        ...
        x_ln = self.ln(x_t2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)  # [B, L, N, H]
...
class STGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        # 模型结构
        self.st_conv1 = STConvBlock(config, self.Ks, self.Kt, self.num_nodes,
                                    self.blocks[0], self.drop_prob, self.Lk, self.w_adj, self.device)
        self.st_conv2 = STConvBlock(config, self.Ks, self.Kt, self.num_nodes,
                                    self.blocks[1], self.drop_prob, self.Lk, self.w_adj, self.device)
    ...
    def predict(self, batch):
        ...
        for i in range(self.output_window):
            ...
            x_ = torch.cat([x_[:, 1:, :, :], y_], dim=1)  # 把预测完的y步添加到x后面。
