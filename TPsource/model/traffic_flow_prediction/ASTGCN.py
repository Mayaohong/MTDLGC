...
class SpatialAttentionLayer(nn.Module):
    ...
    def __init__(self, device, in_channels, num_of_vertices, num_of_timesteps):
        ...
        # for param in [self.bs, self.Vs]:
        #     nn.init.xavier_normal_(param)
        
        nn.init.xavier_normal_(self.W1.unsqueeze(0))
        nn.init.xavier_normal_(self.W2)
        nn.init.xavier_normal_(self.W3.unsqueeze(0))
        nn.init.xavier_normal_(self.Vs)
        nn.init.constant_(self.bs,0)
...
class TemporalAttentionLayer(nn.Module):
    def __init__(self, device, in_channels, num_of_vertices, num_of_timesteps):
        ...
        for param in [self.U1, self.U2, self.U3]:
            if len(param.shape) > 1:
                nn.init.xavier_normal_(param)
            else:
                nn.init.constant_(param,0)

        for param in [self.be, self.Ve]:
            nn.init.xavier_normal_(param)
