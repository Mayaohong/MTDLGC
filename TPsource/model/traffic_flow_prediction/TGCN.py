...
def calculate_normalized_laplacian(adj):
    ...
    normalized_laplacian = d_mat_inv_sqrt.dot(adj).dot(d_mat_inv_sqrt)
    return normalized_laplacian

class TGCNCell(nn.Module):
    def __init__(self, config, num_units, adj_mx, num_nodes, device, input_dim, support, w_adj):
        # ----------------------初始化参数---------------------------#
        super().__init__()
    ...
    def _gc(self, inputs, state, output_size, bias_start=0.0):
        ...
        laplacian = self.normalized_adj
        x0 = x.permute(1, 2, 0)  # (num_nodes, dim+gru_units, batch)
        x0 = x0.reshape((self.num_nodes, -1)) #(N, (H+1)*B)
        x1 = torch.matmul(laplacian, x0)  # A * X
...
class TGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        # 这里提前构建好拉普拉斯
        self.support = (torch.tensor(calculate_normalized_laplacian(self.adj_mx))).type(torch.float32).to(self.device)
        self.w_adj = nn.Parameter(torch.ones(self.num_nodes, self.num_nodes),
                                    requires_grad=True).to(self.device)
        laplacian =  self.support
        # -------------------构造模型-----------------------------
        self.tgcn_model = TGCNCell(config, self.gru_units, self.adj_mx, self.num_nodes, self.device,
                                    self.input_dim, laplacian, self.w_adj)
    ...
    def forward(self, batch):
        ...
        output = output.permute(0, 2, 1, 3)  # [B,L,N,F]
        return output
