...
import random
import numpy as np
from TPsource.config import ConfigParser
from TPsource.data import get_dataset
from TPsource.utils import get_executor, get_model, get_logger, ensure_dir, set_random_seed
...
def run_model(task=None, model_name=None, dataset_name=None, config_file=None,
    ...
    if exp_id is None:
        ...
        exp_id = str(int(random.SystemRandom().random() * 100000))+"_"+model_name + "_"+dataset_name
    ...
    logger.info(config.config)  # 打印config
    ...
    # 转换数据，并划分数据集
    train_data, valid_data, test_data = dataset.get_data()  # train_dataloader, eval_dataloader, test_dataloader
    data_feature = dataset.get_data_feature()  # 在dataset_subclass 中实现了
    # 加载执行器
    model_cache_file = './TPsource/cache/{}/model_cache/{}_{}.m'.format(
...
def hyper_parameter(task=None, model_name=None, dataset_name=None, config_file=None, space_file=None,
    ...
    # ray tune run
    ensure_dir('./TPsource/cache/hyper_tune')
    result = tune.run(tune.with_parameters(train, experiment_config=experiment_config, train_data=train_data,
                      ...
                      local_dir='./TPsource/cache/hyper_tune', num_samples=num_samples)
    ...
    model_cache_file = './TPsource/cache/{}/model_cache/{}_{}.m'.format(
        exp_id, model_name, dataset_name)
    ensure_dir('./TPsource/cache/{}/model_cache'.format(exp_id))
