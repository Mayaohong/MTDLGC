...
class ConfigParser(object):
    ...
    def _parse_config_file(self, config_file):
        if config_file is not None:
            ...
            if os.path.exists('./user_config/{}.json'.format(config_file)):
                with open('./user_config/{}.json'.format(config_file), 'r') as f:
                    x = json.load(f)
                    for key in x:
    ...
    def _load_default_config(self):
        # 首先加载 task config
        with open('./TPsource/config/task_config.json', 'r') as f:
            ...
            if self.config['model'].upper() in ['LSTM']:#'LSTM'
        ...
        for file_name in default_file_list:
            with open('./TPsource/config/{}'.format(file_name), 'r') as f:
                x = json.load(f)
                for key in x:
                    if key not in self.config:
                        self.config[key] = x[key]
