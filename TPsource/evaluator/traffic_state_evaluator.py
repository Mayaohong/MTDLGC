...
import datetime
import pandas as pd
from TPsource.utils import ensure_dir
from TPsource.model import loss
from logging import getLogger
from TPsource.evaluator.abstract_evaluator import AbstractEvaluator
            ...
            for i in range(1, self.len_timeslots+1):
                for metric in self.metrics:
                    if metric == 'masked_MAE':
                        ...
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mape_torch2(y_pred[:, :i], y_true[:, :i]).item())
                    elif metric == 'R2':
                        self.intermediate_result[metric + '@' + str(i)].append(
            ...
            for i in range(1, self.len_timeslots + 1):
                for metric in self.metrics:
                    if metric == 'masked_MAE':
                        ...
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mape_torch2(y_pred[:, i-1], y_true[:, i-1]).item())
                    elif metric == 'R2':
                        self.intermediate_result[metric + '@' + str(i)].append(
